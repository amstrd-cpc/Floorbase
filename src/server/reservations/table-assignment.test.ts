...
test('chooseBestTableSet falls back to combinable tables when no single table fits', () => {
  const choice = chooseBestTableSet(
    [
      { id: 'a', label: 'A', capacityMin: null, capacityMax: 4, isActive: true, canCombine: true, combineGroup: 'g1' },
      { id: 'b', label: 'B', capacityMin: null, capacityMax: 4, isActive: true, canCombine: true, combineGroup: 'g1' },
      { id: 'c', label: 'C', capacityMin: null, capacityMax: 6, isActive: true, canCombine: false, combineGroup: null }
    ],
    7
  );

  assert.deepEqual(choice.map((table) => table.id), ['a', 'b']);
});

test('chooseBestTableSet excludes below-minimum table from combination', () => {
  const choice = chooseBestTableSet(
    [
      { id: 'a', label: 'A', capacityMin: 4, capacityMax: 4, isActive: true, canCombine: true, combineGroup: 'g1' },
      { id: 'b', label: 'B', capacityMin: null, capacityMax: 2, isActive: true, canCombine: true, combineGroup: 'g1' },
      { id: 'c', label: 'C', capacityMin: null, capacityMax: 2, isActive: true, canCombine: true, combineGroup: 'g1' }
    ],
    3
  );

  assert.deepEqual(choice.map((table) => table.id), ['b', 'c']);
});
